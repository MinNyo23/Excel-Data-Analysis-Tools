  ...
  return (
    <SidebarProvider>
      <Sidebar collapsible="icon" className="!border-r-2 !border-[#bfd5bf] !bg-[#e8f3e8] shadow-[8px_0_28px_rgba(30,93,77,0.12)]">
        ...
        <SidebarContent className="px-2 !bg-[#e8f3e8]">
          ...
          <SidebarMenu>
            {tools.map(tool => {
              ...
              return <SidebarMenuItem key={tool.path}><SidebarMenuButton isActive={active} tooltip={tool.label} onClick={() => setLocation(tool.path)} className="h-10 text-[#445149] data-[active=true]:bg-[#e4f1e7] data-[active=true]:text-[#0f6a51]"><tool.icon size={16} /><span>{tool.label}</span></SidebarMenuButton></SidebarMenuItem>;
            })}
          </SidebarMenu>
        </SidebarContent>
        ...
        <SidebarFooter className="px-2 pb-4 pt-0"><SidebarMenu><SidebarMenuItem><SidebarMenuButton isActive={location === "/profile"} tooltip="My profile" onClick={() => setLocation("/profile")} className="h-10 text-[#445149] data-[active=true]:bg-[#e4f1e7] data-[active=true]:text-[#0f6a51]"><UserRound size={16}/><span>My profile</span></SidebarMenuButton></SidebarMenuItem><SidebarMenuItem><SidebarMenuButton isActive={location === "/account"} tooltip="Account management" onClick={() => setLocation("/account")} className="h-10 text-[#445149] data-[active=true]:bg-[#e4f1e7] data-[active=true]:text-[#0f6a51]"><UserRoundCog size={16}/><span>Account management</span></SidebarMenuButton></SidebarMenuItem></SidebarMenu></SidebarFooter>
      </Sidebar>
