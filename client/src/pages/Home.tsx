  ...
  return (
    <main className={`app-shell tool-app-shell tool-${activeTool}`}>
      ...
      <section className="hero container">
        <div className="hero-copy"><Badge className="soft-badge">EXCEL OPERATIONS WORKSPACE</Badge><h2>Choose a workflow<br /><em>from the menu.</em></h2><p>Each tool has its own upload, preview, and download workspace. Select the process you need from the clearly visible navigation menu on the left.</p><details className="privacy-details" open><summary>Your file privacy</summary><p>Your browser sends the selected workbook to the processing request as temporary data. The server reads it in memory, creates the preview and download workbook in memory, then returns the result to your browser. This application does not write uploaded workbooks or their spreadsheet data to its database or file storage.</p><figure className="privacy-diagram"><figcaption><strong>How your upload is processed</strong><span>This diagram shows the validated path from your browser to in-memory analysis and back to your local download.</span><a href="/manus-storage/backend-upload-to-output-flow_81585e5f.png" target="_blank" rel="noreferrer">Open the full-size diagram</a></figcaption><div className="privacy-diagram-viewport"><img src="/manus-storage/backend-upload-to-output-flow_81585e5f.png" alt="Backend processing sequence: authenticated CSV or XLSX uploads are validated, processed in memory by Python pandas and openpyxl, returned to the browser for preview and download, and only optional metadata is saved to the database." loading="lazy" /></div><p className="privacy-diagram-note">The database receives optional account and process metadata only. It does not receive the uploaded workbook, spreadsheet cells, preview rows, or generated Excel file.</p></figure></details></div>
      ...
      <footer className="container footer"><span>MASTER FILE WORKFLOW</span><span>Output tabs: <strong>Summary Report</strong> · <strong>Addition</strong> · <strong>Deletion</strong></span></footer>
    </main>
  );
}
