...
export default function Admin() {
  ...
  const users = useMemo(() => (usersQuery.data as AdminUser[] | undefined ?? []).filter((user: AdminUser) => user.email.toLowerCase().includes(search.toLowerCase())), [usersQuery.data, search]);
  const totals = users.reduce((a: { workflows: number; files: number; records: number }, user: AdminUser) => ({ workflows: a.workflows + user.workflows, files: a.files + user.files, records: a.records + user.records }), { workflows: 0, files: 0, records: 0 });
  ...
  return <div className="admin-page"><div className="admin-heading"><div><span className="soft-badge"><ShieldCheck size={14} /> MASTER ACCOUNT</span><h1>User management</h1><p>Review account activity and manage access from one protected console.</p></div><div className="admin-search"><Search size={16} /><Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search email" aria-label="Search users by email" /></div></div>
    ...
    <div className="admin-table-wrap">{usersQuery.isLoading ? <p className="admin-empty">Loading users…</p> : usersQuery.error ? <p className="admin-empty">Could not load users. Check the master account configuration.</p> : users.length === 0 ? <p className="admin-empty">No users match your search.</p> : <table className="admin-table"><thead><tr><th>Email</th><th>Status</th><th>Workflows</th><th>Files</th><th>Records</th><th>Last activity</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{users.map((user: AdminUser) => <tr key={user.id}><td><strong>{user.email}</strong><small>{user.emailConfirmed ? "Verified" : "Unverified"}</small></td><td><span className={user.bannedUntil ? "admin-status banned" : "admin-status active"}>{user.bannedUntil ? "Banned" : "Active"}</span></td><td>{user.workflows}</td><td>{user.files}</td><td>{user.records.toLocaleString()}</td><td>{user.lastActivity ? new Date(user.lastActivity).toLocaleDateString() : "—"}</td><td className="admin-actions">{user.bannedUntil ? <Button variant="outline" size="sm" onClick={() => act(user.id, "unban", user.email)}><UserCheck size={14} /> Unban</Button> : <Button variant="outline" size="sm" onClick={() => act(user.id, "ban", user.email)}><Ban size={14} /> Ban</Button>}<Button variant="outline" size="sm" className="admin-delete" onClick={() => act(user.id, "delete", user.email)}><Trash2 size={14} /> Delete</Button></td></tr>)}</tbody></table>}</div>
  </div>;
}
